...
- 本地素材库：管理员可在允许根目录内选择素材目录，服务端索引、复制、切片并供混剪匹配使用
- 基于脚本、线上候选和本地素材的线上混剪任务 API，FFmpeg 可用时直接生成 MP4
- 混剪工作台可通过下拉框选择 Microsoft Edge TTS 旁白音色，FFmpeg 可用时会把镜头旁白合成到最终 `output.mp4`，并把音色配置保存到任务 `options` 和 `manifest.json`
- BGM 管理已支持上传、分类、试听、重命名和删除；混剪工作台会保存 BGM 曲目、分类和音量配置，并在 FFmpeg 可用时把选中 BGM 混入最终 `output.mp4`
...
尚未接入登录、权限管理、个人网盘导入、Fish Speech 音色复刻和功能提取处理。
...
前端 `任务与输出` 页会读取最近 50 条任务，展示任务状态、创建/更新时间、素材数量、画幅、分辨率、中文渲染状态和后端汇总的输出摘要；只有主输出为 `output.mp4` 且媒体类型为 `video/mp4` 时显示 `下载视频`，manifest-only 任务显示 `下载清单`，字幕烧录失败等部分输出会展示失败原因并标记输出未完成。下载地址来自 `GET /api/tasks/{task_id}/output`。每个任务卡片提供删除按钮，确认后会调用 `DELETE /api/tasks/{task_id}` 移除任务记录并清理该任务的输出目录。

前端 `音色中心` 页会读取 Microsoft Edge TTS 音色，默认展示中文音色，可切换语言、搜索音色、调整语速/音量/音高并生成 MP3 试听。混剪工作台会用下拉框读取同一套音色列表，支持选择旁白音色并生成当前旁白试听；创建混剪任务时会按镜头旁白生成 Edge TTS 音频并合成到最终 MP4。Edge TTS 路径不需要额外 API key；Fish Speech 只在 `AUTOVIDEO_FISH_SPEECH_URL` 配置后显示为可用。
...
前端 `素材库` 页对应本地素材库能力。服务端只接受 `AUTOVIDEO_MATERIAL_ALLOWED_ROOTS` 配置过的允许根目录，用户在页面中选择某个根目录下的相对目录后，后台会扫描视频文件，把原始视频复制到 `AUTOVIDEO_DATA_DIR/materials/raw`，再用 FFmpeg 切片到 `AUTOVIDEO_DATA_DIR/materials/segments`，并登记原始文件、片段、处理状态和失败摘要。索引库中的删除、单条删除和清空只删除 AutoVideo 数据目录下的复制文件、切片文件和数据库记录，不会删除外部原始目录。清空接口必须提交确认字符串 `CLEAR_MATERIAL_LIBRARY`，缺失或不匹配时返回 `MATERIAL_LIBRARY_CLEAR_CONFIRMATION_REQUIRED`。当前第一阶段 AI 字段用于占位，`asr_status`、`ocr_status`、`vision_status`、`content_label_status` 和 `embedding_status` 会显示为 `not_configured` 或 `skipped`，不代表已经完成语音识别、OCR、视觉理解或向量化。FFmpeg 或 ffprobe 不可用时，索引任务会以结构化素材索引失败结束，例如 `MATERIAL_FFMPEG_UNAVAILABLE`，而不是静默跳过。

混剪工作台的 `material_source_mode` 控制自动选材来源：`local` 只使用已就绪的本地素材库片段；`hybrid` 先匹配本地素材，缺口再走线上免费素材；`online_free` 只使用 Pexels/Pixabay 线上免费素材。使用 `local` 或 `hybrid` 时，如果本地素材库还没有可用片段或索引任务仍在排队/运行，后端会返回结构化错误，前端据此提示先完成素材库索引。
...
- `GET /api/material-sources`：读取本地素材库允许根目录、当前目录源和最新索引任务。未配置 `AUTOVIDEO_MATERIAL_ALLOWED_ROOTS` 时返回 `MATERIAL_SOURCE_ROOT_NOT_CONFIGURED`。
- `PUT /api/material-sources/current`：在允许根目录中保存当前本地素材目录，字段为 `allowed_root_id` 和 `source_relative_path`；路径必须留在允许根目录内，成功后会复用或创建索引任务。
- `POST /api/material-index/jobs`：为当前或指定本地素材目录创建索引任务，可传 `force=true` 重新索引；同一目录已有运行中任务时返回 `MATERIAL_INDEX_ALREADY_RUNNING`。
- `GET /api/material-index/jobs/{job_id}`：查看索引任务状态、阶段、进度、原始文件数、片段数、失败数和错误摘要。
- `GET /api/material-index/summary`：查看本地素材库汇总数量、当前目录源和最新任务。
- `GET /api/material-index/raw-files?limit=50&offset=0&status=failed`：分页查看已索引原始素材，可按状态过滤；响应包含公开展示路径、时长、画幅、片段数、处理状态和第一阶段 AI 状态字段。
- `GET /api/material-index/raw-files/{raw_file_id}/segments`：查看某个原始素材切出的片段，包含片段时长、画幅、匹配文本、AI 状态和失败摘要。
- `DELETE /api/material-index/raw-files/{raw_file_id}`：从 AutoVideo 本地素材库删除单条复制素材、切片文件和登记记录；不会删除外部原始目录中的源视频。
- `POST /api/material-index/library/clear`：清空 AutoVideo 本地素材库复制文件、切片文件和登记记录；必须提交 `{"confirm":"CLEAR_MATERIAL_LIBRARY"}`，不会删除外部原始目录。
...
- `POST /api/online-mix/tasks`：基于结构化脚本创建线上混剪任务。默认推荐
  `asset_strategy=auto`，`material_source_mode` 可为 `local`、`hybrid` 或
  `online_free`，服务端会把脚本里的所有镜头都纳入任务，并按每个镜头的
  `keywords` 或 `visual_description` 自动匹配本地素材库片段或搜索、下载线上免费素材。用户也可以为部分镜头
  传入 `shot_assets` 或 `shot_materials` 作为覆盖项；线上候选只提交
  `candidate_token`，服务端验签后重新解析 provider 下载地址并保存素材；本地素材只提交
  真实 `material_id`。`options` 可包含 `voice_id`、`voice_name`、`voice_provider`、
  `voice_locale` 和 `voice_gender`，用于保存旁白音色配置并在 FFmpeg 可用时合成镜头旁白音频；首版仅支持 `edge_tts`。
  `options` 可包含 `bgm_enabled`、`bgm_track_id`、
  `bgm_category_id` 和 `bgm_volume`，用于保存 BGM 配置；只选择分类时，任务创建阶段会
  解析为当前分类中的具体曲目快照，并在 FFmpeg 可用时把选中 BGM 按音量混入最终 MP4。FFmpeg 可用时任务主输出为 MP4 视频，同时任务目录会保存
...
- `AUTOVIDEO_MATERIAL_ALLOWED_ROOTS`：本地素材库允许根目录，格式为
  `id=path;another_id=path`。`id` 是 API 和页面展示使用的根目录标识，`path` 必须是
  后端进程可读取的目录；留空时本地素材库目录选择功能禁用。示例：
  `demo=./sample-materials`。不要把包含真实用户名、内网地址、token、key 或密码的路径写入仓库。
