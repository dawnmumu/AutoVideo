...
SAFE_BOOLEAN_STATUS_KEYS = frozenset({"candidate_token_secret_configured"})
...
def sanitize_manifest_payload(value: Any) -> Any:
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            key_text = str(key)
            if SENSITIVE_MANIFEST_KEY_RE.search(key_text):
                if key_text in SAFE_BOOLEAN_STATUS_KEYS and isinstance(item, bool):
                    sanitized[key] = item
                continue
