...
6. 所有镜头时长之和要接近目标总时长
7. 必须紧扣用户输入的标题/主题，不要改成其他行业、产品或故事
8. 不要套用固定商品广告模板；只有用户明确给出卖点或转化目标时，才写产品亮点或行动号召
9. 主题生成时采用：开场场景 → 核心信息 → 细节/变化 → 结果收束
...
TOPIC_CORE_TOKEN_RE = re.compile(r"[\u4e00-\u9fff]+|[A-Za-z0-9]+")
CHINESE_TOPIC_TOKEN_RE = re.compile(r"^[\u4e00-\u9fff]+$")
...
def _split_chinese_topic_terms(text: str) -> list[str]:
    if len(text) <= 3:
        return [text]
    if len(text) == 4:
    ...
    while cursor < len(text):
        ...
        if remaining <= 3:
            terms.append(text[cursor:])
            break
        terms.append(text[cursor : cursor + 2])
        cursor += 2
    return terms


def _extract_topic_core_terms(topic: str, stopwords: set[str]) -> list[str]:
    terms: list[str] = []
    seen: set[str] = set()
    for match in TOPIC_CORE_TOKEN_RE.finditer(str(topic or "")):
        token = match.group(0)
        candidates = (
            _split_chinese_topic_terms(token)
            if CHINESE_TOPIC_TOKEN_RE.fullmatch(token)
            else [token]
        )
...
def _contains_topic_term(normalized_text: str, term: str) -> bool:
    cursor = 0
    while term and cursor < len(normalized_text):
...
def _topic_relevance_groups(topic: str) -> list[list[str]]:
    ...
    if not normalized_topic or _is_placeholder_title(topic):
        return []
    ...
    def normalize_group(values: list[str]) -> list[str]:
        terms: list[str] = []
    ...
    def add_group(values: list[str]) -> None:
        terms = normalize_group(values)
        if not terms:
            return
        group_key = tuple(terms)
    ...
    for key, aliases in sorted(
        TOPIC_RELEVANCE_ALIASES.items(),
        key=lambda item: len(_normalize_compare_text(item[0])),
        reverse=True,
    ):
        normalized_key = _normalize_compare_text(key).lower()
        if normalized_key and normalized_key in normalized_topic:
            before_count = len(groups)
            add_group([key, *aliases])
...
def text_matches_topic(text: str | None, topic: str | None) -> bool:
    ...
    matched_groups = [
        group
        for group in groups
        if any(_contains_topic_term(normalized_text, term) for term in group)
    ]
    required_matches = 1 if len(groups) == 1 else 2
